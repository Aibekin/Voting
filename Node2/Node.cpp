...
// перегрузка для сокета
void Node::handleMessage(const nlohmann::json& message, SOCKET clientSocket) {
    if (!message.contains("type") || !message.contains("data")) {
    ...
    if (type == "TRANSACTION") {
        try {
            ...
            // Проверка подписи
            EVP_PKEY* pubKey = loadPublicKey(vote.publicKey);
            if (!vote.verify(pubKey)) {
                EVP_PKEY_free(pubKey);
                throw std::runtime_error("Invalid vote signature");
            }
            EVP_PKEY_free(pubKey);

            // Проверка существования кошелька
            std::string storedPubKey;
            if (!WalletDB::walletExists(vote.voterId, &storedPubKey)) {
                throw std::runtime_error("Voter not registered");
            }

            // Конвертация в транзакцию
            Transaction tx(vote);
            nlohmann::json txMessage;
            txMessage["type"] = "TRANSACTION";
            txMessage["data"] = tx.toJson();

            // Обработка как обычной транзакции
            handleMessage(txMessage, nullptr);
            ...
            std::string response = "{\"status\":\"error\",\"message\":\"" + std::string(e.what()) + "\"}\n";
            send(clientSocket, response.c_str(), response.size(), 0);
        }
    }
    else {
        std::string response = "{\"status\":\"error\",\"message\":\"Unknown message type\"}\n";
        send(clientSocket, response.c_str(), response.size(), 0);
    }
}
